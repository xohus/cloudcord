...
export const gitHash = process.env.SINCORD_HASH || execSync("git rev-parse HEAD", { encoding: "utf-8" }).trim();
...
/**
 * @type {(kind: "web" | "discordDesktop" | "vesktop" | "sinbop") => import("esbuild").Plugin}
 */
export const globPlugins = kind => ({
    ...
    setup: build => {
        ...
        build.onLoad({ filter, namespace: "import-plugins" }, async () => {
            const pluginDirs = ["plugins/_api", "plugins/_core", "plugins", "sincordplugins/_api", "sincordplugins/_core", "sincordplugins", "userplugins"];
            let code = "";
            let pluginsCode = "\n";
            let metaCode = "\n";
            let excludedCode = "\n";
                ...
                for (const file of files) {
                    ...
                    if (target && !IS_REPORTER) {
                        const excluded =
                            ...
                            (target === "vesktop" && kind !== "vesktop" && kind !== "sinbop") ||
                            (target === "sinbop" && kind !== "sinbop" && kind !== "vesktop");
...
export const gitRemotePlugin = {
    ...
    setup: build => {
        ...
        build.onLoad({ filter, namespace: "git-remote" }, async () => {
            let remote = process.env.SINCORD_REMOTE;
            if (!remote) {
