...
import { ChromiumIcon, ClientIcon, DevBannerIcon, DiscordIcon, ElectronIcon, SincordIcon, names, settings } from ".";
...
export function makeDevBanner(state?: string): string | JSX.Element {
    ...
    const replaced = baseFormat
        ...
        .replace(/{sinbopHash}/g, clientInfo.shortHash ?? "Not Supported")
        .replace(/{sinbopPlatform}/g, `v${clientInfo?.dev ? "Dev Build" : "Standalone"}`)
    ...
    const parts = replaced.split(/({.*?}|__NEWLINE__)/).filter(Boolean).map((part, i) => {
        switch (part) {
            ...
            case "{sincordIcon}":
                return <span key={`icon-sincord-${i}`} className="vc-discord-dev-banner-icons"><SincordIcon /></span>;
            case "{electronIcon}":
