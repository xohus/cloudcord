...
Other CloudCord clients discover a profile from the invisible `CCP1` marker that the CloudCord client maintains in the user's real Discord About Me. Because the marker is delivered by Discord with that user's real profile, the service does not allow arbitrary user-ID claims.
