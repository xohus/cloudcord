...
import { isCloudCordGuild, isCloudCordSupport } from "@utils/misc";
...
export function ChatPluginCard({ url, description }: { url: string, description: string; }) {
    ...
    if (excludedPlugin || !p) {
        const toolTipText = excludedPlugin
            ...
            : "This plugin is not on this version of CloudCord. Try updating!";
...
export const PluginCards = ErrorBoundary.wrap(function PluginCards({ message }: { message: Message; }) {
    ...
    message.embeds?.forEach(embed => {
        if (!embed.url?.startsWith("https://cloudcord.xohus.lol/plugins") && !embed.url?.startsWith("https://vencord.dev/plugins/")) return;

        const isCloudCord = isCloudCordGuild(message.channel_id) && isCloudCordSupport(message.author.id);
        if (!isCloudCord) return;
    ...
    if (message.author.id === EQUIBOT_USER_ID && components?.length >= 4) {
        ...
        if (pluginUrl?.startsWith("https://cloudcord.xohus.lol/plugins") || pluginUrl?.startsWith("https://vencord.dev/plugins/")) {
