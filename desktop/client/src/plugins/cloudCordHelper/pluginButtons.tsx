...
import { isSincordGuild, isSincordSupport } from "@utils/misc";
...
export const PluginButtons = ErrorBoundary.wrap(function PluginCards({ message }: { message: Message; }) {
    ...
    const isSincord = isSincordGuild(message.channel_id) && isSincordSupport(message.author.id);
    const startsWithEnabled = msg.startsWith("enable");
    const startsWithDisabled = msg.startsWith("disable");

    const shouldAddPluginButtons = pluginData && isSincord && (startsWithEnabled || startsWithDisabled);
