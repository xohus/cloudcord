...
func main() {
	...
	var installFlag = flag.Bool("install", false, "Install CloudCord")
	var updateFlag = flag.Bool("repair", false, "Repair CloudCord")
	var uninstallFlag = flag.Bool("uninstall", false, "Uninstall CloudCord")
	...
	if !SliceContainsFunc(switches, func(b *bool) bool { return *b }) {
		...
		go func() {
			...
			if IsSelfOutdated {
				...
				Log.Warn("To update, select the 'Update CloudCord Setup' option to update, or run with --update-self")
			}
		}()
	...
	if install {
		...
		Log.Info("Downloading latest CloudCord files...")
