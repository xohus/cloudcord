...
if (!IS_VANILLA) {
     ...
     * on vesktop and sinbop because they manage their own updates.
     */
    if (!IS_VESKTOP && !IS_EQUIBOP) {
        try {
            require("./hostUpdateHook").installHostUpdateHook();
        } catch (err) {
            console.error("[CloudCord] Failed to install host update hook", err);
        }
    }
    ...
    process.env.DATA_DIR = join(app.getPath("userData"), "..", "CloudCord");
    ...
    console.log("[CloudCord] Running in vanilla mode. Not loading CloudCord");
}

console.log("[CloudCord] Loading original Discord app.asar");
