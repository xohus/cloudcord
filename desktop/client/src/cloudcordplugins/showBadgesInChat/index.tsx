...
import { isCloudCordPluginDev, isPluginDev } from "@utils/misc";
...
function CheckBadge({ badge, author }: { badge: string; author: User; }): JSX.Element | null {
    ...
    switch (badge) {
        case "CloudCordDonor":
            return (
                <span style={{ order: settings.store.CloudCordDonorPosition }}>
                    {badges.getCloudCordDonorBadges(author.id)?.map(badge => (
                        <RoleIconComponent
                            key={author.id}
                            className={roleIcon}
        ...
        case "CloudCordContributer":
            return isCloudCordPluginDev(author.id) ? (
                <span style={{ order: settings.store.CloudCordContributorPosition }}>
                    <RoleIconComponent
                        className={roleIcon}
                        name="CloudCord Contributor"
                        size={20}
...
function ChatBadges({ author }: { author: User; }) {
    ...
    return (
        <span className="vc-sbic-badge-row" style={{ margin: "2px" }}>
            {settings.store.showCloudCordDonor && <CheckBadge badge={"CloudCordDonor"} author={author} />}
            {settings.store.showCloudCordContributor && <CheckBadge badge={"CloudCordContributer"} author={author} />}
...
export default definePlugin({
    ...
    authors: [Devs.Inbestigator, CloudCordDevs.KrystalSkull],
