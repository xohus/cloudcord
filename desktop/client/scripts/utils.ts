...
export interface PluginData {
    ...
    target: "discordDesktop" | "vesktop" | "sinbop" | "desktop" | "web" | "dev";
    filePath: string;
    dirName: string;
    isModified: boolean;
}
...
export function parseCloudCordDevs() {
    const file = createSourceFile("constants.ts", readFileSync("src/utils/constants.ts", "utf8"), ScriptTarget.Latest);
...
export async function parseFile(fileName: string) {
    ...
    for (const node of file.getChildAt(0).getChildren()) {
        ...
        if (target) {
            if (!["web", "discordDesktop", "vesktop", "sinbop", "desktop", "dev"].includes(target)) throw fail(`invalid target ${target}`);
